...
### 变式训练卷配图规则
**重要**：以后做变式训练卷时，原卷习题中**有的配图必须全部配上**，一个都不能漏。
- 原卷发送过来后，先识别哪些题目有配图
- 生成变式卷时，这些配图必须一起出现在对应题目位置
- 如果原图格式不支持（如WMF矢量图），想替代方案或明确告知用户

### 本次完成情况
- ✅ Q6脑啡肽结构式、Q16葡萄糖分解图、Q27遗传系谱图、Q11细胞核模式图、Q18细胞周期图：已配原图
- ⚠️ Q32氨基酸代谢途径图：原图为WMF矢量格式（848字节），系统无法渲染，已用简化示意图代替
...
### 变式训练卷配图规则
**重要**：以后做变式训练卷时，原卷习题中有的配图必须全部配上，一个都不能漏。
- 原卷发送过来后，先识别哪些题目有配图
- 生成变式卷时，这些配图必须一起出现在对应题目位置
- 如果原图格式不支持（如WMF矢量图），想办法替代或告知用户

### 本次完成情况
- Q6脑啡肽结构式、Q16葡萄糖分解图、Q27遗传系谱图、Q11细胞核模式图、Q18细胞周期图：已配原图
- Q32氨基酸代谢途径图：原图为WMF矢量格式（848字节）无法渲染，已用简化示意图代替
