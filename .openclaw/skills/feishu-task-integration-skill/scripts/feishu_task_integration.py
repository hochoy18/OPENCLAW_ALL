...
class FeishuTaskManager:
    ...
    def load_config_from_file(self):
        """从配置文件加载凭证"""
        config_file = os.path.expanduser('~/.openclaw/workspace/feishu_config.json')
        try:
    ...
    def get_user_tasks(self, user_id, completed=False):
        """获取用户任务 - 使用 user_access_token"""
        # 优先使用 user_access_token
        if not self.user_access_token:
            print("⚠️  未找到 user_access_token，尝试使用 tenant_token...")
            if not self.tenant_access_token:
                if not self.get_tenant_access_token():
                    return []
