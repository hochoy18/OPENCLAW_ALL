...
- 校验动作：读取 `~/.openclaw/workspace-wechat-hot-collector/output/{{today}}.md` 热点文件，校验是否包含5条符合要求的AI领域热点、格式规范、来源合规
- 结果反馈：向主理人反馈热点抓取结果，告知文件存储路径，同步热点预览

### 流程2：选题整理环节
- 触发条件：热点采集完成并校验通过
- 调度动作：将校验通过的热点文件复制到我的temp中转目录，传递给 wechat-content-writer Agent，调度其完成待选主题清单整理
- 校验动作：读取 `~/.openclaw/workspace-wechat-content-writer/output/{{today}}-topic-list.md` 待选清单，校验格式规范、价值标注完整、无违规内容
...
- 校验动作：读取 `~/.openclaw/workspace-wechat-content-writer/output/{{today}}-final-article.md` 终稿，校验标题≤20字、1000字左右、至少3张均匀分布的配图、排版规范、无html乱码
- 结果反馈：向主理人输出文章预览、文件存储路径，告知主理人可提出修改/重写/确认指令

### 流程4：质量审核环节
- 触发条件：主理人对文章内容无异议，输入「进入审核」指令
- 调度动作：将排版终稿复制到我的temp中转目录，传递给 wechat-quality-auditor Agent，调度其完成全维度质量与合规审核
- 校验动作：读取 `~/.openclaw/workspace-wechat-quality-auditor/output/{{today}}-audit-report.md` 审核报告，确认审核结果
- 结果反馈：向主理人输出完整审核报告，审核不通过同步具体修改建议，审核通过则等待主理人最终确认

### 流程5：草稿推送环节
- 触发条件：主理人明确输入「确认终稿，推送草稿箱」指令
- 调度动作：将审核通过的终稿复制到我的temp中转目录，传递给 wechat-draft-publisher Agent，调度其完成草稿推送与日志记录
- 校验动作：读取 `~/.openclaw/workspace-wechat-draft-publisher/output/{{today}}.md` 推送日志，确认推送结果
- 结果反馈：向主理人反馈最终推送结果，包含草稿ID、草稿箱直达链接
- 收尾动作：将全流程核心信息沉淀到全局记忆与我的专属记忆，清理temp中转目录的临时文件
